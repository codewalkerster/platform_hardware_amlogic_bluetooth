...
int upio_power_get(void)
{
    ...
    if (init_rfkill())
    {
        ALOGE("upio_power_get failed");
        return -1;
    }
    ALOGD("rfkill_state_path : %s", rfkill_state_path);
    fd = open(rfkill_state_path, O_RDONLY);
    if (fd < 0)
    {
        ALOGE("upio_power_get open failed");
        return -1;
    }

    sz = read(fd, &buffer, 1);
    ALOGD("upio_power_get read %d", sz);
    if (sz < 0)
    {
        ALOGD("%s: Can't read: %s", __FUNCTION__, strerror(errno));
        close(fd);
        return 0;
    }
    if (buffer == '0')
    ...
    close(fd);
    ALOGD("upio_power_get buffer %c , ret %d", buffer, ret);
    return ret;
}
