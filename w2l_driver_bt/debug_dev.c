...
void amlbt_debug_send_cmd(debug_dev_t *d_bt)
{
    ...
    {
        if (d_bt->w2l_dev->usb_irq_task_quit)
        {
            BTE("bt close \n");
            return ;
        }
        mutex_lock(&d_bt->w2l_dev->bt_debug_mutex);
        t_fifo = *d_bt->w2l_dev->tx_cmd_fifo;
        ro_fw_evt_fifo = *d_bt->w2l_dev->fw_evt_fifo;
        ro_fw_type_fifo = *d_bt->w2l_dev->fw_type_fifo;
        //send cmd
        if (d_bt->w2l_dev->tx_cmd_fifo == NULL)
        {
            BTE("%s: tx_cmd_fifo NULL!!!!\n", __func__);
            mutex_unlock(&d_bt->w2l_dev->bt_debug_mutex);
            return ;
        }
        ...
        *d_bt->w2l_dev->tx_cmd_fifo = t_fifo;
        BTP("len %#x:w %#lx, r %#lx\n", d_bt->_cmd.hci_len, (unsigned long)d_bt->w2l_dev->tx_cmd_fifo->w, (unsigned long)d_bt->w2l_dev->tx_cmd_fifo->r);

        //get data
        p_rs_inf(sram_buf, (unsigned char *)(unsigned long)DBG_USB_EVENT_Q_ADDR, TOTAL_LEN, USB_EP2);
        d_bt->w2l_dev->fw_type_fifo->w = (unsigned char *)(unsigned long)((sram_buf[35]<<24)|(sram_buf[34]<<16)|(sram_buf[33]<<8)|sram_buf[32]);
        d_bt->w2l_dev->fw_evt_fifo->w = (unsigned char *)(unsigned long)((sram_buf[39]<<24)|(sram_buf[38]<<16)|(sram_buf[37]<<8)|sram_buf[36]);

        while (wait_cnt < 20)
        {
            if (d_bt->w2l_dev->fw_type_fifo->w != d_bt->w2l_dev->fw_type_fifo->r
                                            && d_bt->w2l_dev->fw_evt_fifo->w != d_bt->w2l_dev->fw_evt_fifo->r)
            {
                //get type
                memset(type_buff, 0, sizeof(type_buff));
                read_fifo.base_addr = &sram_buf[DBG_RX_TYPE_ADDR - DBG_USB_EVENT_Q_ADDR];
                read_fifo.r = d_bt->w2l_dev->fw_type_fifo->r;
                read_fifo.w = d_bt->w2l_dev->fw_type_fifo->w;
                ...
                {
                    BTE("%s:%d type error!\n", __func__, __LINE__);
                    BTE("fw_type_fifo->w:%#x fw_type_fifo->r:%#x", d_bt->w2l_dev->fw_type_fifo->w, d_bt->w2l_dev->fw_type_fifo->r);
                    BTE("fw_evt_fifo->w:%#x fw_evt_fifo->r:%#x", d_bt->w2l_dev->fw_evt_fifo->w, d_bt->w2l_dev->fw_evt_fifo->r);
                    mutex_unlock(&d_bt->w2l_dev->bt_debug_mutex);
                    return ;
                }

                d_bt->w2l_dev->fw_type_fifo->r = read_fifo.r;
                ...
                read_fifo.r = d_bt->w2l_dev->fw_evt_fifo->r;
                read_fifo.w = d_bt->w2l_dev->fw_evt_fifo->w;
                ...
                BTP("evt fifo:w %#lx, r %#lx\n", (unsigned long)d_bt->w2l_dev->fw_evt_fifo->w, (unsigned long)d_bt->w2l_dev->fw_evt_fifo->r);
                if (evt_size)
                {
                    d_bt->w2l_dev->fw_evt_fifo->r = read_fifo.r;
                    ...
                    if (read_len != 0)
                    {
                        memcpy(d_bt->_cmd.hci_evt, &fw_read_buff[4], 4);
                    }
                    if (d_bt->_cmd.hci_evt[4] == d_bt->_cmd.hci_cmd[0] &&
                                            d_bt->_cmd.hci_evt[5] == d_bt->_cmd.hci_cmd[1])
                    {
                        reg = (((unsigned int)(unsigned long)d_bt->w2l_dev->fw_evt_fifo->r) & 0x1fff);
                        read_reg[4] = (reg & 0xff);
                    ...
                    {
                        BTE("event not match \n");
                        *d_bt->w2l_dev->fw_evt_fifo = ro_fw_evt_fifo;
                        *d_bt->w2l_dev->fw_type_fifo = ro_fw_type_fifo;
                    }
                }
            }
...
static long amlbt_debug_ioctl(struct file* filp, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd)
    {
        ...
        case IOCTL_GET_BT_VERSION:
        {
            ...
            if (FAMILY_TYPE_IS_W2L(amlbt_if_type))
            {
                memcpy(d_bt->_cmd.ver_inf, AML_W2LU_VERSION, strlen(AML_W2LU_VERSION));
                memcpy(d_bt->_cmd.manf_inf, d_bt->w2l_dev->rc_manfdata, d_bt->w2l_dev->manfdata_len);
                d_bt->_cmd.manf_len = d_bt->w2l_dev->manfdata_len;
            }
            //amlbt_show_debug();
            if (copy_to_user((void __user *)arg, &d_bt->_cmd, sizeof(d_bt->_cmd)) != 0)
            {
...
int amlbt_debug_dev_init(ws_inf p_ws_func, rs_inf p_rs_func, ww_inf p_ww_func, rw_inf p_rw_func, void *bt_dev)
{
    ...
    else
    {
        ...
        if (FAMILY_TYPE_IS_W2L(amlbt_if_type))
        {
            if (bt_dev != NULL)
            {
                debug_dev.w2l_dev = (w2l_usb_bt_new_t *)bt_dev;
            }
            else
            {
